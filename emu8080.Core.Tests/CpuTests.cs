...
namespace emu8080.Core.Tests
{
    public class CpuTests
    {
        ...
        public void DAD_SP()
        {
            ...
            cpu.Registers.H = 0x12;
            cpu.Registers.L = 0x15;
            cpu.Registers.StackPointer = 0x4271;

            cpu.Step(memory);

            cpu.Registers.HL.Should().Be(0x5486);
            cpu.Registers.Flags.Carry.Should().BeFalse();
            cpu.Registers.ProgramCounter.Should().Be(1);
        }
        ...
        public void DCX()
        {
            ...
            cpu.Registers.HL = 0x72;
            cpu.Step(memory);
            cpu.Registers.HL.Should().Be(0x71);
            cpu.Registers.ProgramCounter.Should().Be(2);
        }
        ...
        public void MOV_R1_R2()
        {
            ...
            cpu.Registers.H = 0;
            cpu.Registers.B = 0xe8;
            cpu.Step(memory);
            cpu.Registers.H.Should().Be(0xe8);
            cpu.Registers.ProgramCounter.Should().Be(29);

            cpu.Registers.H = 0xe8;
            cpu.Step(memory);
            cpu.Registers.H.Should().Be(0xe8);
            cpu.Registers.ProgramCounter.Should().Be(30);

            cpu.Registers.L = 0xe8;
            cpu.Step(memory);
            cpu.Registers.L.Should().Be(0xe8);
            cpu.Registers.ProgramCounter.Should().Be(31);

            cpu.Registers.B = 0xe8;
            cpu.Registers.A = 0;
            cpu.Step(memory);
            cpu.Registers.A.Should().Be(0xe8);
            cpu.Registers.ProgramCounter.Should().Be(32);

            cpu.Registers.A = 0x42;
            cpu.Step(memory);
            cpu.Registers.A.Should().Be(0x42);
            cpu.Registers.ProgramCounter.Should().Be(33);
        }
        ...
        public void ADC()
        {
            ...
            cpu.Registers.Flags.Carry = false;
            cpu.Step(memory);
            cpu.Registers.A.Should().Be( (byte)(0xa3 + memory[0]));
            cpu.Registers.ProgramCounter.Should().Be(11);
            ...
            cpu.Registers.Flags.Carry = true;
            cpu.Step(memory);
            cpu.Registers.A.Should().Be( (byte)(0xa7 + memory[0] + 1));
            cpu.Registers.ProgramCounter.Should().Be(12);
        }
        ...
        public void RET()
        {
            ...
            // RPE
            cpu.Registers.ProgramCounter = 6;
        ...
        public void JMP()
        {
            ...
            cpu.Registers.ProgramCounter = 0;
            cpu.Registers.Flags.Reset();
            cpu.Registers.Flags.CalcSignFlag(0xff);
            cpu.Step(memory);
            cpu.Registers.ProgramCounter.Should().Be(3);
        ...
        public void PUSH()
        {
            ...
            cpu.Registers.ProgramCounter = 0;
            cpu.Registers.HL = 0x3164;
            cpu.Registers.StackPointer = 3000;
            cpu.Step(memory);
            cpu.Registers.StackPointer.Should().Be(2998);
            memory[cpu.Registers.StackPointer + 1].Should().Be(0x31);
            memory[cpu.Registers.StackPointer].Should().Be(0x64);
            cpu.Registers.ProgramCounter.Should().Be(1);
            ...
            cpu.Registers.ProgramCounter = 0;
            cpu.Registers.A = 0x42;
            cpu.Registers.StackPointer = 5000;
            cpu.Registers.Flags.PSW = 0x71;
            cpu.Step(memory);
            cpu.Registers.StackPointer.Should().Be(4998);
            memory[cpu.Registers.StackPointer + 1].Should().Be(0x42);
            memory[cpu.Registers.StackPointer].Should().Be(0x71);
            cpu.Registers.ProgramCounter.Should().Be(1);
        }
        ...
        public void ANI()
        {
            ...
            cpu.Step(memory);
            cpu.Registers.A.Should().Be(0x0a);
            cpu.Registers.Flags.Carry.Should().BeFalse();
            cpu.Registers.Flags.AuxCarry.Should().BeFalse();
            cpu.Registers.Flags.Zero.Should().BeFalse();
            cpu.Registers.Flags.Parity.Should().BeTrue();
        ...
        public void XTHL()
        {
            ...
            cpu.Step(memory);
            cpu.Registers.ProgramCounter.Should().Be(1);
            cpu.Registers.HL.Should().Be(0x4431);
            memory[1].Should().Be(0x71);
        ...
        public void CPI()
        {
            ...
            cpu.Step(memory);
            cpu.Registers.A.Should().Be(0x4a);
            cpu.Registers.Flags.Carry.Should().BeFalse();
            cpu.Registers.Flags.AuxCarry.Should().BeTrue();
            cpu.Registers.Flags.Zero.Should().BeFalse();
            cpu.Registers.Flags.Parity.Should().BeTrue();
